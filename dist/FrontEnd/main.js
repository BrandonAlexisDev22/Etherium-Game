...
body === null || body === void 0 ? void 0 : body.appendChild(escenario);
...
Juego.equipo1.forEach((personajes, i) => {
    ...
    if (personajes == Juego.personajeActual) {
        ...
        if (contenedorHabilidades1 && 'style' in contenedorHabilidades1)
            contenedorHabilidades1.style.display = "block";
        if (contenedorHabilidades2 && 'style' in contenedorHabilidades2)
...
const personajesHTML = [
    ...Array.from(document.querySelectorAll(".personaje")),
    ...Array.from(document.querySelectorAll(".personaje2")),
];
function ActualizarVida() {
    personajesObjetos.forEach(pOBJ => {
        personajesHTML.find(pHTML => {
            if (String(pOBJ.id) === pHTML.id) {
                const parrafo = pHTML.querySelector('.parrafo1');
                if (parrafo)
                    parrafo.textContent = `${pOBJ.vida}/100`;
                const vidaDiv = pHTML.querySelector('.vida');
                if (vidaDiv && vidaDiv.lastElementChild)
                    vidaDiv.lastElementChild.setAttribute('style', `width: ${pOBJ.vida}%`);
            }
        });
    });
}
function EfectoMuerto() {
    personajesObjetos.forEach(pOBJ => {
        personajesHTML.find(pHTML => {
            if (String(pOBJ.id) === pHTML.id) {
                if (pOBJ.vida == 0) {
                    const img = pHTML.querySelector('.imagen1');
                    if (img)
                        img.classList.add('efectoFantasma');
                }
            }
        });
    });
...
function ActualizarAtaque() {
    personajesObjetos.forEach(pOBJ => {
        personajesHTML.find(pHTML => {
            if (String(pOBJ.id) === pHTML.id) {
                const parrafo = pHTML.querySelector('.parrafo2');
                if (parrafo)
                    parrafo.textContent = String(pOBJ.ataque);
            }
        });
    });
}
function ActualizarDefensa() {
    personajesObjetos.forEach(pOBJ => {
        personajesHTML.find(pHTML => {
            if (String(pOBJ.id) === pHTML.id) {
                const parrafo = pHTML.querySelector('.parrafo3');
                if (parrafo)
                    parrafo.textContent = String(pOBJ.defensa);
            }
        });
    });
}
        ...
        personajesHTML.find(pHTML => {
            if (String(pOBJ.id) === pHTML.id) {
                if (pOBJ.vida == 0) {
                    ...
                    const venenoDiv = pHTML.querySelector('.efectoVeneno');
                    if (venenoDiv)
                        venenoDiv.style.display = 'none';
                    const quemadoDiv = pHTML.querySelector('.efectoQuemado');
                    if (quemadoDiv)
                        quemadoDiv.style.display = 'none';
                }
            }
        });
    });
...
function ActualizarHabilidades() {
    ...
    if (contenedorHabilidades1 instanceof HTMLElement)
        contenedorHabilidades1.style.display = "none";
    if (contenedorHabilidades2 instanceof HTMLElement)
        contenedorHabilidades2.style.display = "none";
        ...
        personajesHTML.find(pHTML => {
            if (pHTML.id === String(pOBJ.id)) {
                if (pOBJ === Juego.personajeActual) {
                    if (pHTML.firstElementChild instanceof HTMLElement)
...
let habilidadSeleccionada = undefined;
...
function animacion(personajeActual, personaje) {
    ...
    const ubicacionObj = ubicacionPersonajeObj ? ubicacionPersonajeObj.querySelector('.imagen2') : null;
    const Personaje = document.getElementById(String(personajeActual.id));
    const ubicacionPersonaje = Personaje ? Personaje.querySelector('.imagen1') : null;
    const personajeDañado = ubicacionPersonajeObj ? ubicacionPersonajeObj.querySelector('.imagen1') : null;
    if (ubicacionPersonaje)
    ...
    personajesObjetos.forEach(pOBJ => {
        personajesHTML.find(pHTML => {
            if (String(pOBJ.id) === pHTML.id) {
                ...
                const venenoDiv = pHTML.querySelector('.efectoVeneno');
                if (venenoDiv)
                    venenoDiv.style.display = Veneno ? 'block' : 'none';
                const Quemado = pOBJ.debilitamiento.find(efecto => efecto.nombre == 'Quemar');
                const quemadoDiv = pHTML.querySelector('.efectoQuemado');
                if (quemadoDiv)
                    quemadoDiv.style.display = Quemado ? 'block' : 'none';
            }
        });
    });
}
