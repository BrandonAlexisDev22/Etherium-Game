...
class Personaje {
    ...
    Atacar(objetivo) {
        ...
        if (Juego.personajeActual != this)
            return console.warn("es el turno de", Juego.personajeActual); // si el personaje con el turno actual no es el que acciona la habilidad no se efectua la habilidad
        if (this.equipo === objetivo.equipo)
            return console.warn('No puedes atacar a tu compañero!');
    ...
    usarHabilidad(nombreHabilidad, objetivo) {
        ...
        if (objetivo.estaMuerto())
            return console.warn("El personaje", objetivo.nombre, "esta muerto");
        ; // si el objetivo esta muerto no se podra aplicar la habilidad
        if (Juego.personajeActual != this)
            return console.warn("Es el turno de", Juego.personajeActual); // si el personaje con el turno actual no es el que acciona la habilidad no se efectua la habilidad
        const habilidadUsarIndex = this.habilidades.findIndex((habilidad) => habilidad.Nombre == nombreHabilidad); //Encuentra el indice de la habilidad
        if (habilidadUsarIndex === -1)
            return console.warn("Habilidad no encontrada"); // si la habilidad no es encontrada no se podra usar
        const habilidadUsar = this.habilidades[habilidadUsarIndex]; // La habilidad a usar
        if (habilidadUsar.Espera != 0)
            return console.warn("la habilidad esta en espera por", habilidadUsar.Espera, "turnos");
        ; // en caso de que la habilidad este en cooldown no se podra usar 
